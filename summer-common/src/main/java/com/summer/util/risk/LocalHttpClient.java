...
public class LocalHttpClient {
    ...
    public static <T> T execute(HttpUriRequest request, ResponseHandler<T> responseHandler) {
        ...
        if (responseHandler instanceof AbstractLocalResponseHandler) {
            AbstractLocalResponseHandler lrh = (AbstractLocalResponseHandler) responseHandler;
            lrh.setUriId(uriId);
        }*/
        try {
     ...
     * @return
     */
    public static <T> T executeGzipPost(String url, String jsonStringParams, Class<T> clazz) {

        try {
            ByteArrayOutputStream originalContent = new ByteArrayOutputStream();
            originalContent
                    .write(jsonStringParams.getBytes(Charset.forName("UTF-8")));
            ...
            RequestBuilder requestBuilder = RequestBuilder
                    ...
                    .setEntity(new ByteArrayEntity((baos.toByteArray())))
                    .setHeader("Content-Type", CommonConstant.CONTENT_TYPE_JSON)
                    .setUri(url);
            ...
            logger.info("模型调用异常：{}", e.getMessage());
            e.printStackTrace();
            return null;
        }
     ...
     * @return
     */
    public static <T> T executeGzipSLPPost(String url, String jsonStringParams, Class<T> clazz) {
