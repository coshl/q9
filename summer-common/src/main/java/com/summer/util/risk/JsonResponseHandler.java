...
public class JsonResponseHandler {
    ...
    public static <T> ResponseHandler<T> createResponseHandler(final Class<T> clazz) {
        return new JsonResponseHandlerImplAbstract<T>(null, clazz);
    }

    public static class JsonResponseHandlerImplAbstract<T> extends AbstractLocalResponseHandler implements ResponseHandler<T> {

        private Class<T> clazz;

        public JsonResponseHandlerImplAbstract(String uriId, Class<T> clazz) {
            this.uriId = uriId;
            this.clazz = clazz;
        }
        ...
        public T handleResponse(HttpResponse response)
            ...
            if (status >= 200 && status < 300) {
                ...
                logger.info("URI[{}] elapsed time:{} ms RESPONSE DATA STATUS:{}",
                        super.uriId + (System.currentTimeMillis() - super.startTime), status);
